    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant={"outline"}
          className={cn(
            "w-[200px] justify-start px-0 text-left text-xl font-semibold",
        ...
        >
          <div className="flex w-full items-center gap-2">
            {background ? (
              <div
                className="h-9 w-full rounded !bg-cover !bg-center transition-all"
                style={{ background }}
              ></div>
            ) : (
              <Paintbrush className="mx-auto" />
            )}
            {!background && placeholder && (
              <div className="flex-1 truncate">{placeholder}</div>
            )}
          </div>
        </Button>
      </PopoverTrigger>
